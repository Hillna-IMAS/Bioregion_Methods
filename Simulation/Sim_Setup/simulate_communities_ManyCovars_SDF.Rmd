---
title: "Simulate Communities to compare bioregionalisation methods"
output:
...
## Simulate species that have grouped response to their environment

* Using multivariate normal mixture model code from Skip
...
* Also added species specific intercepts instead of group means (with some variance)
...
```{r generate sim data, echo=TRUE}
## Load up the simulated data    
...
#variances <- matrix(c(rep(c(0.05,0.05,0.05),each=dim(means)[2])),dim(means)[1],dim(means)[2],byrow=T)
...
legend("topright", legend=paste0("Group ", 1:3),  col=1:length( unique( sim_data$group))+1, pch=20)
...
# hard class groups
hc<-apply(true_grps2,1, which.max)
