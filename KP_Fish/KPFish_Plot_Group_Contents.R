...
rcp_contents_SD<-dotplot_sp_tab (mean_df=t(rcp_calc_contents$mean),
                      ...
                      nGrp=4, species= species, method="RCP Coeficients")
...
gtp<-dotplot_sp_tab(mean_df = matrix(NA,nrow=20, ncol=4),
                    error_df = matrix(0,nrow=20, ncol=4),
                    nGrp=4, species=species, method="Two-stage: Group then Predict")
ptg<-dotplot_sp_tab(mean_df = matrix(NA,nrow=20, ncol=4),
                    error_df = matrix(0,nrow=20, ncol=4),
                    nGrp=4, species=species, method="Two-stage: Predict then Group")
os<-dotplot_sp_tab(mean_df = matrix(NA,nrow=20, ncol=4),
                   error_df = matrix(0,nrow=20, ncol=4),
                   nGrp=4, species=species, method="One-stage")

contents_SE<-rbind( do.call(rbind, hclust_contents_SE),
                   rcp_contents_SE, gtp, ptg, os)


contents_SE$Method<-mapvalues(contents_SE$Method ,from=c("RCP_Hard", "BioHC_RF"), to=c("RCP (Hard Class)", "BioHC_RF (Hard Class)" ))

contents_SE$Method<-factor(contents_SE$Method, 
                            levels=c("RCP (Hard Class)", "RCP Coeficients",  "One-stage", 
...
contents_SE$species<-rep(pretty_sp, 56)
...
p<-ggplot(data = contents_SE,
  ...
  scale_colour_manual(name="Method", 
                      values=c( "darkblue",  "cornflowerblue", "white", 
...
tiff(file="Results/Plots/Grp_sp_SE.tiff", height=16, width=20, units="cm", res=1000)
