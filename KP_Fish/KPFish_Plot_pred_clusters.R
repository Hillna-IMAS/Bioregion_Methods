...
mods<-c("Env_Only", "SpRF_HC","HMSC_HC" , "MNet_HC" , "GDM_Dissim_HC","GDM_TransEnv_HC",  "bbGDM_Dissim_HC"  , "bbGDM_TransEnv_HC",  "GF_HC")
check_clust2<-list()
for (i in 1: length(mods)){
  check_clust2[[i]]<- table(hard_cluster_opt$SpRF_HC, hard_cluster_opt[,mods[i]])
}
names(check_clust2)<-mods

# attempt to make labels match up 
...
hard_cluster_opt$HMSC_HC<-mapvalues(hard_cluster_opt$HMSC_HC, from=c(1,2), to=c(2,1)) 
hard_cluster_opt$GDM_Dissim_HC<-mapvalues(hard_cluster_opt$GDM_Dissim_HC, from=c(1,2), to=c(2,1)) 
hard_cluster_opt$GDM_TransEnv_HC<-mapvalues(hard_cluster_opt$GDM_TransEnv_HC, from=c(1,2,3), to=c(2,3,1)) 
#hard_cluster_opt$bbGDM_Dissim_HC<-mapvalues(hard_cluster_opt$bbGDM_Dissim_HC, from=c(1,2), to=c(2,1)) 
hard_cluster_opt$bbGDM_TransEnv_HC<-mapvalues(hard_cluster_opt$bbGDM_TransEnv_HC, from=c(1,2,3), to=c(2,3,1)) 
...
rat<-data.frame(ID=1:4, Group=paste0("Bioregion ", 1:4))
...
class_pal<-c("darkolivegreen4","grey", "orange1", "darkred")


#plot hard cluster outputs
p.strip <- list(cex=0.6, lines=1)

tiff(filename="Results/Plots/Opt_HClusts.tiff", compression="lzw", 
     width=11, height=15, units="cm", res=1000)
levelplot(clust_opt, layout=c(2,5),scales=list(draw=FALSE), 
...
levelplot(rasterize(pred_sp[,1:2], env_raster,field= bio2_rf_pred), 
         par.settings=YlOrRdTheme, names.attr=c("Bioregion 1", "Bioregion 2"),
          layout=c(2,1), scales=list(draw=FALSE))
dev.off()

#SAM
#account for label switching
dimnames(sam4_boot_pred$ptPreds)[[2]]<- c(paste0("Bioregion_", c(2,4,1,3)))
dimnames(sam4_boot_pred$bootSEs)[[2]]<- c(paste0("Bioregion_", c(2,4,1,3)))
bioregions<-paste0("Bioregion_",1:4)
...
levelplot(rasterize(pred_sp[,1:2], env_raster, 
          field=sam4_boot_pred$ptPreds[,paste0("Bioregion_", 1:4)]), 
          col.regions=prob_pal(19),
          at=seq(0,1,length=18),
          colorkey=list(at=seq(0,1,length=18),col=prob_pal(19)),
          names.attr=paste0("Archetype ",1:4),
          layout=c(2,2),par.strip.text=p.strip,
          scales=list(draw=FALSE))
dev.off()

#RCP
dimnames(rcp5_spat_preds[["ptPreds"]])[[2]]<- c(paste0("Bioregion ", c(1,2,4,5,3)))
dimnames(rcp5_spat_preds[["bootSEs"]])[[2]]<- c(paste0("Bioregion ", c(1,2,4,5,3)))

tiff(filename="Results/Plots/RCP_prob.tiff", compression="lzw", 
     width=10, height=8, units="cm", res=1000)
levelplot(rasterize(pred_sp[,1:2], env_raster,field=rcp5_spat_preds$ptPreds[,paste0("Bioregion ", 1:5)]), 
          col.regions=prob_pal(19),
          at=seq(0,1,length=18),
          colorkey=list(at=seq(0,1,length=18),col=prob_pal(19)),
          names.attr=rep(" ",5),
...
levelplot(clust4,layout=c(3,4),scales=list(draw=FALSE), 
          col.regions=class_pal[1:4],par.strip.text=p.strip)
dev.off()

#####################################################
### Plot uncertainty for SAMs and RCPS
hard_cluster4$RCP<-mapvalues(hard_cluster4$RCP ,from=c(1,2,3,4), to=c(2,3,4,1))
dimnames(rcp4_spat_preds$bootSEs)[[2]]<-paste0("Bioregion ", c(2,3,4,1))
...
plot(levelplot(rasterize(pred_sp[,1:2], env_raster,field=sam4_boot_pred[["bootSEs"]][,paste0("Bioregion_", 1:4)]), 
    layout=c(4,1), xlab="",ylab="",
    col.regions=prob_pal,
    colorkey=list(at=seq(0,1,length=18),col=prob_pal(19)),
    names.attr=paste0("Bioregion ", 1:4),
    scales=list(draw=FALSE), par.strip.text=p.strip,
    main="SAM"), 
     split=c(1,1,1,2))

plot(levelplot(rasterize(pred_sp[,1:2], env_raster,field=rcp4_spat_preds[["bootSEs"]][,paste0("Bioregion ", 1:4)]), 
     layout=c(4,1), xlab="", ylab="",
     col.regions=prob_pal,
     colorkey=list(at=seq(0,1,length=18),col=prob_pal(19)),
     #colorkey=FALSE,
...
######################################################
## Comaprison of GDM and bbGDM clustering approaches
######################################################

tiff(file="Results/Plots/Cluster4_GDM_comp.tiff",compression="lzw", 
     width=12, height=12, units="cm", res=1000)
levelplot(clust4, layers=c(6,7,8,9),layout=c(2,2),scales=list(draw=FALSE), 
          col.regions=class_pal[1:4],par.strip.text=p.strip)
dev.off()
...
tiff(file="Results/Plots/SAM_probs2.tiff",compression="lzw", 
     width=14, height=10, units="cm", res=1000)
levelplot(rasterize(pred_sp[,1:2], env_raster, field=sam4_pred$fit[,paste0("Bioregion ", 1:4)]), 
          col.regions=prob_pal(19),
          at=seq(0,1,length=18),
          colorkey=list(at=seq(0,1,length=18),col=prob_pal(19)),
...
dimnames(rcp4_spat_preds[["ptPreds"]])[[2]]<- c(paste0("Bioregion ", c(2,3,4,1)))
dimnames(rcp4_spat_preds[["bootSEs"]])[[2]]<- c(paste0("Bioregion ", c(2,3,4,1)))

tiff(file="Results/Plots/RCP_prob2.tiff",compression="lzw", 
     width=14, height=10, units="cm", res=1000)
levelplot(rasterize(pred_sp[,1:2], env_raster,field=rcp4_spat_preds$ptPreds[,paste0("Bioregion ", 1:4)]), 
          col.regions=prob_pal(19),
          at=seq(0,1,length=18),
          colorkey=list(at=seq(0,1,length=18),col=prob_pal(19)),
